...
function get_alpha_xmax(p_vector,prob)
    ...
    return (t_grid_alpha,dyn_alpha),(t_plot,(level_x_wt_rescaled,level_x_cp_rescaled,level_x_lm))
end
...
function get_alpha_xmax_binned(p_vector,prob)
    ...
    return (t_grid_alpha,dyn_alpha),(t_plot,(level_x_wt_rescaled,level_x_cp_rescaled,level_x_lm)),c_level, wt_t0, sol,sol_cp
end
...
function metric_loss_v1(p_vector,prob,data_metrics,alpha_data)
    ...
    return (wt_d0 - data_metrics[:wt_d0])^2 + (cp_d0 - data_metrics[:cp_d0])^2 + (xmax_peak_ratio - data_metrics[:xmax_peak_ratio])^2 + alpha_mse  + ((wt_xMax/60) - 1)^2 + ((cp_xMax/80) - 1)^2 
end
