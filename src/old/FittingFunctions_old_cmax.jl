...
function get_summary_metrics_fix_endT(p_vector,prob,xmax_data,alpha_data,max_t)
...
function get_alpha_xmax(p_vector,prob,max_t)
    ...
    return (t_grid_alpha,dyn_alpha),(t_plot,(level_x_wt_rescaled,level_x_cp_rescaled,level_x_lm)),(porosity_dyn,porosity_dyn_cp),c_level
...
function get_alpha_xmax_lambda_binned(p_vector,prob,cp)
    ...
    c_max_wt = mean(sol(λhalf_max_t)[1:10,1])
...
function loss_components(p_vector,prob,xmax_data,alpha_data,cp)
    ...
    c_level = cp*min(c_max_cp,c_max_wt)
    # c_level = 0.2*c_max_wt
...
function metric_loss_v1(p_vector,prob,data_metrics,alpha_data,cp)
