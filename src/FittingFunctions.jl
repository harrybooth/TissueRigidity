...
function get_summary_metrics(p_vector,prob,xmax_data,alpha_data)
    ...
    sol = solve(prob, p = p, FBDF(),abstol = 1e-8,reltol = 1e-6, maxiters = 1e6,callback = TerminateSteadyState(1e-6,1e-4),isoutofdomain = (u,p,t) -> any(x->x<0, u));
    sol_cp = solve(prob, p = p_cp, FBDF(),abstol = 1e-8,reltol = 1e-6,maxiters = 1e6,callback = TerminateSteadyState(1e-6,1e-4),isoutofdomain = (u,p,t) -> any(x->x<0, u));
    sol_lm = solve(prob, p = p_lm, FBDF(),abstol = 1e-8,reltol = 1e-6,maxiters = 1e6,callback = TerminateSteadyState(1e-6,1e-4),isoutofdomain = (u,p,t) -> any(x->x<0, u));
