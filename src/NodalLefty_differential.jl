...
function nodal_lefty_spatial_diff!(du,u,p,t)
    ...
    dα[1] =  softplus(-(E[1] + α0^(-1))^(-2)*(ν(N[1],σE,NE,1) - kE*E[1]),1e4)
    ...
    @inbounds for j in 2:Nc-1
        ...
        dα[j] =  softplus(-(E[j] + α0^(-1))^(-2)*(ν(N[j],σE,NE,1) - kE*E[j]),1e4)
    end
    ...
    dα[Nc] =  softplus(-(E[Nc] + α0^(-1))^(-2)*(ν(N[Nc],σE,NE,1) - kE*E[Nc]),1e4)
